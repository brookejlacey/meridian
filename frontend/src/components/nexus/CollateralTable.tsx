          ...
          {(["deposit", "withdraw"] as const).map((a) => (
            <button
              ...
              className={`text-xs px-2 py-1 rounded transition-colors ${
                action === a ? "bg-blue-500 text-white" : "bg-zinc-800 text-zinc-400 hover:text-white"
              }`}
            >
      ...
      {action && (
        <div className="mt-2 flex gap-2">
          <input
            ...
            placeholder="Amount"
            className="flex-1 px-2 py-1 text-sm bg-zinc-900 border border-zinc-700 rounded focus:border-blue-500 outline-none"
          />
          <button
            onClick={handleSubmit}
            disabled={isPending || isApproving}
            className="px-3 py-1 text-sm bg-blue-600 hover:bg-blue-500 disabled:bg-zinc-700 disabled:text-zinc-500 rounded transition-colors"
          >
    ...
    <div className="p-3 rounded-lg border border-[var(--card-border)] bg-[var(--card-bg)]">
      ...
      <div className="flex gap-2 flex-wrap">
        <input
          ...
          className="flex-1 min-w-48 px-2 py-1 text-sm bg-zinc-900 border border-zinc-700 rounded focus:border-blue-500 outline-none"
        />
        <input
          type="text" value={amount} onChange={(e) => setAmount(e.target.value)}
          placeholder="Amount"
          className="w-32 px-2 py-1 text-sm bg-zinc-900 border border-zinc-700 rounded focus:border-blue-500 outline-none"
        />
        <button
          onClick={handleSubmit}
